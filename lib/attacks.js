...
class Attacks {
  ...
  hitPerson(){
    let p1 = this.stage.children[0].getTransformedBounds();
    let p2 = this.stage.children[1].getTransformedBounds();
    let curShuriken = this.bitmap;
    //Cat is 22 pixels wide, tranformed bounds has center x
    let hitp1 = ((curShuriken.x > p1.x - 11) && (curShuriken.x < p1.x +11) &&
                  (curShuriken.y > p2.y) && (curShuriken.y < p2.y + 96)
                  );
    let hitp2 = ((curShuriken.x > p2.x - 11) && (curShuriken.x < p2.x + 11) &&
                  (curShuriken.y > p2.y) && (curShuriken.y < p2.y + 96)
                  );
    // console.log(hitp1);
    // console.log(hitp1);
